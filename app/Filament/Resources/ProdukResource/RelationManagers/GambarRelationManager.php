...
class GambarRelationManager extends RelationManager
{
  ...
  protected static ?string $recordTitleAttribute = 'alt_text_2222336';
    ...
    return $form
      ->schema([
        Forms\Components\FileUpload::make('path_img_2222336')
          ...
          ->visibility('public')
          ->required(),
        Forms\Components\TextInput::make('alt_text_2222336')
          ->label('Teks Alternatif (Alt Text)')
          ->required()
          ->maxLength(255),
        Forms\Components\Toggle::make('is_main_2222336')
          ->label('Gambar Utama')
          ...
          ->afterStateUpdated(function ($state, $set, $record, $livewire) {
            if ($state) {
              ...
              $produk->path_img_2222336 = $record->path_img_2222336;
              $produk->save();
              ...
              $livewire
                ->getOwnerRecord()
                ->gambar()
                ->where('id_2222336', '!=', $record->id_2222336)
                ->update(['is_main_2222336' => false]);
            }
          }),
        Forms\Components\TextInput::make('sort_order_2222336')
          ->label('Urutan')
          ->numeric()
          ->default(0)
    ...
    return $table
      ->columns([
        Tables\Columns\ImageColumn::make('path_img_2222336')
          ...
          ->visibility('public')
          ->square(),
        Tables\Columns\TextColumn::make('alt_text_2222336')
          ->label('Alt Text')
          ->searchable()
          ->limit(30),
        Tables\Columns\IconColumn::make('is_main_2222336')
          ->label('Gambar Utama')
          ->boolean(),
        Tables\Columns\TextColumn::make('sort_order_2222336')
          ->label('Urutan')
          ->sortable(),
      ...
      ->defaultSort('sort_order_2222336')
      ->filters([
        //
      ])
      ->headerActions([
        Tables\Actions\CreateAction::make()
          ->using(function (array $data, $livewire): mixed {
            $data['produk_id_2222336'] = $livewire->getOwnerRecord()->id_2222336;

            // Jika ini gambar utama, update gambar utama di produk
            if (isset($data['is_main_2222336']) && $data['is_main_2222336'] === true) {
              $produk                   = $livewire->getOwnerRecord();
              $produk->path_img_2222336 = $data['path_img_2222336'];
              $produk->save();

              // Set semua gambar lain menjadi bukan utama
              $produk->gambar()->update(['is_main_2222336' => false]);
            }
      ...
      ->actions([
        ...
        Tables\Actions\DeleteAction::make()
          ->before(function ($record, $livewire) {
            ...
            if ($record->is_main_2222336) {
              $produk                   = $livewire->getOwnerRecord();
              $produk->path_img_2222336 = null;
              $produk->save();
            }
        ...
        Tables\Actions\Action::make('set_as_main')
          ...
          ->color('success')
          ->hidden(fn($record) => $record->is_main_2222336)
          ->action(function ($record, $livewire) {
            // Set sebagai gambar utama
            $record->is_main_2222336 = true;
            $record->save();
        ...
        Tables\Actions\BulkActionGroup::make([
          Tables\Actions\DeleteBulkAction::make()
            ->before(function ($records, $livewire) {
              ...
              if ($records->where('is_main_2222336', true)->count() > 0) {
                $produk                   = $livewire->getOwnerRecord();
                $produk->path_img_2222336 = null;
                $produk->save();
              }
          ...
          Tables\Actions\BulkAction::make('update_order')
            ...
            ->action(function ($records, $data) {
              ...
              foreach ($records as $record) {
                $record->sort_order_2222336 = $startOrder + $i;
                $record->save();
                $i++;
              }
            })
